...
class TestMeasureToViewMap(unittest.TestCase):
    ...
    def test_get_view(self):
        ...
        measure_to_view_map._measure_to_view_data_list_map = {
            view.measure.name: [ViewData(view=view, start_time=timestamp, end_time=timestamp)]}
        ...
        measure_to_view_map._measure_to_view_data_list_map = {
            view.measure.name: [
                ViewData(view=mock.Mock(), start_time=timestamp, end_time=timestamp)]}
        view_data = measure_to_view_map.get_view(view_name=name,
                                                 timestamp=timestamp)
        self.assertIsNone(view_data)
    ...
    def test_filter_exported_views(self):
        ...
        measure = mock.Mock()
        aggregation = mock.Mock()
        test_view_1 = View(name=test_view_1_name, description=description, columns=columns, measure=measure,
                           aggregation=aggregation)
        print("test view 1", test_view_1)

        test_view_2_name = "testView2"
        test_view_2 = View(name=test_view_2_name, description=description, columns=columns, measure=measure,
                           aggregation=aggregation)
    ...
    def test_register_view(self):
        ...
        self.assertIsNotNone(
            measure_to_view_map._measure_to_view_data_list_map[view.measure.name])
        ...
        self.assertIsNone(measure_to_view_map._registered_measures.get(measure.name))
        measure_to_view_map.register_view(view=view, timestamp=timestamp)
        # view is already registered, measure will not be registered again.
        self.assertIsNone(measure_to_view_map._registered_measures.get(measure.name))
        self.assertIsNotNone(measure_to_view_map._measure_to_view_data_list_map[view.measure.name])
    ...
    def test_record(self):
        ...
        view = View(name=view_name, description=view_description, columns=view_columns, measure=view_measure,
        ...
        measure_to_view_map._measure_to_view_data_list_map = {measure.name: [mock.Mock()]}
        measure_to_view_map.record(tags=tags, measurement_map=measurement_map,
                                   timestamp=timestamp)
        self.assertEqual(measure,
                         measure_to_view_map._registered_measures.get(
                             measure.name))
        self.assertTrue(measure.name in measure_to_view_map._measure_to_view_data_list_map)

        measure_to_view_map._measure_to_view_data_list_map = {"testing": [mock.Mock()]}
        measure_to_view_map.record(tags=tags, measurement_map=measurement_map,
                                   timestamp=timestamp)
        self.assertTrue(measure.name not in measure_to_view_map._measure_to_view_data_list_map)
